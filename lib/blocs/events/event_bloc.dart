...
class EventBloc extends Bloc<EventBlocEvent, EventBlocState> {
  ...
  try {
    ...
    EventModel eventModel = EventModel(
      userId: eventDetails.userId, // Thêm userId vào đây
  ...
  Future<void> _onFetchEventsForDay(FetchEventsForDay event, Emitter<EventBlocState> emit) async {
  emit(EventLoading());

  try {
    DateTime startOfDay = DateTime(event.selectedDay.year, event.selectedDay.month, event.selectedDay.day);
  ...
  Future<void> _onDeleteEvent(DeleteEvent event, Emitter<EventBlocState> emit) async {
    emit(EventDeleting());
    try {
  ...
  Future<void> _onUpdateEvent(UpdateEvent event, Emitter<EventBlocState> emit) async {
  emit(EventUpdating());

  try {
    final docSnapshot = await _firestore.collection('events').doc(event.eventId).get();
    if (docSnapshot.exists && docSnapshot['userId'] == event.userId) {
      ...
      await _firestore.collection('events').doc(event.eventId).update(eventModel.toMap());
      emit(EventUpdatedSuccess());
    } else {
      emit(EventUpdatedFailure('Sự kiện không tồn tại hoặc không thuộc về người dùng.'));
    }
